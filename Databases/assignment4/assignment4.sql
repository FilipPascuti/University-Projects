...
go
create or alter view view_getMembers
as
	select *
	from Member
go

create or alter view view_getPastorsWithChurches
as
	select * 
	from Churches Ch inner join Pastors_chuches PC on PC.Church = Ch.ChurchID inner join Pastors P on P.PastorID = PC.Pastor
go

create or alter view view_getMoneyPaidByChurch
as
	select Ch.ChurchID, sum(P.Salary) as Money_paid
	from Churches Ch inner join Pastors_chuches PC on PC.Church = Ch.ChurchID inner join Pastors P on P.PastorID = PC.Pastor
	group by Ch.ChurchID
go
...
--create or alter procedure usp_insert_one_row @table_name varchar(50)
--as
--begin
...
--					set @query = @query + cast(@random_number as varchar(5)) + ','
--				end
--				if @column_type = 'int'
--				begin
...
--				begin
--					set @random_number = floor(rand()*1000)
--					set @random_string = '''random' + cast(@random_number as varchar(4)) + ''''
...
--					set @query = @query + '''' + '1' + cast(@random_number as varchar(6)) + cast(@random_number as varchar(6)) + '''' + ','
--				end
--				if @column_type = 'date'
--				begin
...
--				create table random_fk_table(fk int)
--				exec('insert into random_fk_table select top 1 ' + @refered_column_name + ' from ' + @refered_table_name + ' order by newid()')
--				set @random_fk = (select top 1 fk from random_fk_table)
--				set @query = @query + cast(@random_fk as varchar) + ','
--				drop table random_fk_table
--			end
...
exec usp_create_new_test 'Test1', 'Member,Small_group,Pastors_chuches,Churches,Pastors', 'view_getPastorsWithChurches,view_getMoneyPaidByChurch', 1000
